...
export default function ProfilePage() {
  ...
  const [tab, setTab] = useState("pending"); // To toggle between Approval Pending, Approved, and Pending tabs
  ...
  useEffect(() => {
    const fetchPendingProfile = async () => {
      try {
        ...
        const response = await fetch("https://ecommstagingapi.tboo.com/admin/pending-profiles", requestOptions);
        const result = await response.json();
    ...
    <Layout>
      <div className="flex space-x-8 mb-6 border-b-2 pb-4">
        <button
          ...
          className={`text-xl font-medium ${tab === "pending" ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-600'}`}
        >
          Approval Pending
        </button>
        <button
          onClick={() => setTab("approved")}
          className={`text-xl font-medium ${tab === "approved" ? 'text-green-500 border-b-2 border-green-500' : 'text-gray-600'}`}
        >
          Approved
        </button>
        <button
          onClick={() => setTab("rejected")}
          className={`text-xl font-medium ${tab === "rejected" ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-600'}`}
        >
          Rejected
        </button>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className={`p-4 rounded-lg border ${profileData?.data?.results.aadhar_front_image_url_status === 'approved' ? 'bg-green-100' : 'bg-gray-100'}`}>
              ...
              <ul className='flex flex-col gap-y-2 py-2'>
                <li>
                  <Image width={1000} height={1000} src={profileData?.data?.results.aadhar_front_image_url} alt="Aadhaar Front" className="w-full h-48 object-cover rounded-lg mb-4" />
                </li>
                ...
                <li>Age : <span className='font-bold'> {profileData?.data?.results?.aadhar_api_returned_dob}</span></li>
                {/* <li>Gender :</li> */}
            ...
            <div className={`p-4 rounded-lg border ${profileData?.data?.results.licence_front_image_url_status === 'approved' ? 'bg-green-100' : 'bg-gray-100'}`}>
              ...
              <ul className='flex flex-col gap-y-2 py-2'>
                ...
                <li>Aadhar Number : <span className='font-bold'>{profileData?.data?.results?.licence_number}</span> </li>
                <li>Age : <span className='font-bold'> {profileData?.data?.results?.dl_api_returned_dob}</span></li>
                {/* <li>Gender :</li> */}
                <li>Location : {profileData?.data?.results?.aadhar_response_address}</li>
              </ul>
              <div className="flex flex-col gap-y-3">
      ...
      {
        tab === "approved" &&
        <div className='flex gap-5 pt-4'>{PendingData?.data?.results?.map((item, index) => (
