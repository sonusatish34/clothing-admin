...
const ComponentName = () => {
  ...
  const uploadImg = async (event, key) => {
    ...
    try {
      ...
      await fetch("https://dev.zuget.com/admin/replace-image", {
        method: "POST",
        headers: myHeaders,
        body: JSON.stringify({
  ...
  const fetchAssignStore = async () => {
    const response = await fetch("https://dev.zuget.com/admin/assign-store", {
      method: "POST",
      headers: {
        "Accept": "application/json",
  ...
  const updateStoreStatus = async (newStatus) => {
    try {
      ...
      myHeaders.append("Authorization", localStorage.getItem(`${localStorage.getItem('user_phone')}_token`));
      myHeaders.append("Content-Type", "application/json");
      await fetch("https://dev.zuget.com/admin/update-store-status", {
        method: "PUT",
        headers: myHeaders,
        body: JSON.stringify({
